...
<c:set var="qualificationAttachList" value=" ${board.qualificationAttachList } " />
...
<div class="title_area" style="background-color: #95c1e0;">
	...
	<div class="location">
		...
		<span class="f_nav">시험정보</span> <span class="tit"><strong>검정고시</strong></span>
		<!-- 페이지 입력 밑 -->
	</div>
		...
		<article class="board-text">
			...
			<div class='fieldBox'>
				<dl>
					<dt>첨부파일</dt>
					<dd>
						<c:forEach items="${board.qualificationAttachList }" var="attach">
							<a
								href="<%=request.getContextPath()%>/board/qualificationBoard/getFile.do?ano=${attach.ano }">
								${attach.fileName }</a>
			...
			<c:if test="${loginUser.id eq board.writer }">
			<div class="btnGrp">
				<a href="javascript:Submit_go('<%=request.getContextPath() %>/board/qualificationBoard/modifyForm.do',${board.bno })"> <span>수정</span>
				</a>
				<p class="pt10"></p>
			</div>
			</c:if>
...
function removeCheck() {
	 ...
	 if (confirm("정말 삭제하시겠습니까??") == true){    //확인
		 ...
		 Submit_go('<%=request.getContextPath() %>/board/qualificationBoard/remove.do',${board.bno });
