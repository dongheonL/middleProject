...
<div class="subCntBody clearfix">
	...
	<div class="cntBody clearfix">
		...
		<article class="board-text">
				...
				<dl>
					...
					<dd>
						<c:forEach items="${board.attachList }" var="attach">
							<a
								href="<%=request.getContextPath()%>/board/notificationBoard/getFile.do?ano=${attach.ano }">
								${attach.fileName }</a>
			...
			<c:if test="${loginUser.id eq board.writer }">
			<div class="btnGrp">
				<a href="javascript:Submit_go('<%=request.getContextPath() %>/board/notificationBoard/modifyForm.do',${board.bno })"> <span>수정</span>
				</a>
				<p class="pt10"></p>
			</div>
			</c:if>
...
function removeCheck() {
	 ...
	 if (confirm("정말 삭제하시겠습니까??") == true){    //확인
		 ...
		 Submit_go('<%=request.getContextPath() %>/board/notificationBoard/remove.do',${board.bno });
